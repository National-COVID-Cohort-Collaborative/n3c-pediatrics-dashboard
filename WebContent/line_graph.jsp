...
//set the dimensions and margins of the graph
	...
	d3.json("${param.data_page}", function(error, data2) {	
		...
		function draw_regs() {
			...
			var reg_line = d3.line().x(d => x(d.date)).y(d => y(d.month_n));
			...
			reg_y.domain([0, d3.max(data2, function(d) { return d.month_n; })]);
		  
			// X & Y 
			var reg_valueline = d3.line()
				.x(function(d) { return reg_x(d.date); })
				.y(function(d) { return reg_y(d.month_n); });
...
//			reg_svg.append("text")
//		    	.attr("transform", "translate("+(reg_width+3)+","+reg_y(data2[data2.length-1].month_n)+")")
//		    	.attr("dy", ".35em")
//		    	.attr("text-anchor", "start")
//		    	.attr("class", "registration")
//		    	.text("Users: (" + String(data2[data2.length-1].month_n)+ ")");
...
// 		    	.attr("r", 5);
			...
			function reg_mousemove() {
			    ...
			    reg_focus.select(".tooltip-date").text(d.month_year_factor);
			    reg_focus.select(".tooltip-registration").text(formatValue(d.month_n));
